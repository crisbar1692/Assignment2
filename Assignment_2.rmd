...
pls <- plsr(bp ~ ., ncomp = 4, data = train_set, validation = "LOO")
```
...
plot(pls, ncomp = 3, asp = 1, line = TRUE)
```
<!-- In this point with the model build is time to show the results for the test data -->
```{r}
pred<-predict(pls, ncomp = 4, newdata = test_set)
x<-RMSEP(pls, newdata = test_set)
print(x)
```
